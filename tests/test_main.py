...
class AdvancedTests(unittest.TestCase):
    ...
    def tearDown(self,):
        ...
        cursor.execute("DELETE FROM Students WHERE LecturerID=?", ('testaccount123',))
        cursor.commit()

    def teststudentsignup(self):

        response = self.app.post('/signup', data=dict(idname='160012345', password='password123', fname="John", lname="Smith", type="Student", follow_redirects=True))
    ...
    def lecturersignup(self):

        response = self.app.post('/signup', data=dict(idname='testaccount123', password='password123', fname="Jane", lname="Doe", type="Lecturer", follow_redirects=True))
